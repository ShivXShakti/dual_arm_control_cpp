    ...
    void timerCallback()
    {
        ...
        std::string urdf_string((std::istreambuf_iterator<char>(urdf_file)),
            ...
            for (unsigned int i = 0; i < chain_l.getNrOfJoints(); i++) {
                ql(i) = joint_state_actual_[i]; // radians
            }
            ...
            for (unsigned int i = 0; i < chain_l.getNrOfJoints(); i++) {
                joint_state_actual_lGF[i] = joint_state_actual_[i]; 
            }
            ...
            for (unsigned int i = 0; i < chain_r.getNrOfJoints(); i++) {
                qr(i) = joint_state_actual_[i+7]; // radians
            }
            ...
            for (unsigned int i = 0; i < chain_r.getNrOfJoints(); i++) {
                joint_state_actual_rGF[i] = 0.0;//joint_state_actual_[i+7]; 
            }
            ...
            for (unsigned int i = 0; i < chain_l.getNrOfJoints(); i++) {
                desired_tau[i] = final_torque_l[i]; 
            }
            for (unsigned int i = 0; i < chain_r.getNrOfJoints(); i++) {
                desired_tau[i+7] = final_torque_r[i]; 
            }
            std_msgs::msg::Float64MultiArray gmsg;
            gmsg.data = desired_tau;
            desired_pub_->publish(gmsg);

            for (unsigned int i = 0; i <desired_tau.size(); i++) {
                std::cout << "desired_tau " << i+1 << ": " << desired_tau[i] << " Nm" << std::endl;
            }
        }
